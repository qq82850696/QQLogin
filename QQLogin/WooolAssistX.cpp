	...
	if (OpenPackage())
	{
		...
		for (vectorwstring::iterator it = vArr.begin(); it != vArr.end(); it++)
		{
			...
			if (m_dm.FindStr(_T("经验为"), _T("ffff00-101010")) > -1)
			{
				...
				LOG_INFO << _T("识别的结果:") << retstr.c_str();
...
bool CWooolAssistX::AutoFightStrange(BOOL bAuto)
{
	...
	for (size_t i = 0; i < 10; i++)
	{
		...
		else
		{
			...
			if (0 == dm_ret)
			{
				LOG_INFO << _T("开启自动打怪成功");
				break;
			}
			else if (1 == dm_ret)
			{
				LOG_INFO << _T("取消自动打怪成功");
				break;
			}
		}
		m_an.Delays(30, 50);
...
bool CWooolAssistX::OpenPackage()
{
	...
	do
	{
		...
		else
		{
			...
			{
				LOG_INFO << _T("包裹移到屏幕外去了");

			}
			else
			{
				if (db)
					LOG_INFO << _T("包裹打开失败,重试中...");
				db = true;
				...
				if (CValidateUtil::IsDate(gTime.c_str()))
				{
					...
					{
						LOG_INFO << _T("<打开包裹>游戏不刷新");
					}
					else
					{
						LOG_INFO << _T("<打开包裹>游戏正常");
					}
				}
				else {
					LOG_INFO << "<打开包裹>时间识别错误:" << gTime.c_str();
				}
