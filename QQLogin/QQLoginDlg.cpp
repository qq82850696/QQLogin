...
BOOL CQQLoginDlg::OnInitDialog()
{
    ...
    try
    {
        ...
        /*  auto hwnd = m_an.FindWindowW(_T("Notepad++"), _T("Notepad++"));
          if (hwnd > 0)
          {
              auto hwndex = m_an.FindWindowEx(hwnd, _T("Scintilla"), nullptr);
              if (hwndex > 0)
              {
                  m_an.SetWindowState(hwndex, 12);
                  KPress(_T("Wo.Cao-Ni*904302"));
              }
          }*/
...
void CQQLoginDlg::SaveSetting()
{
    ...
    UpdateData(TRUE);
    LPTSTR pstr = nullptr;
    qQLoginHistory._QQStartInterval = _tcstol(m_strInterval.GetBuffer(), &pstr, 10);
    ...
    if (qqHwnd > 0)
    {
        ...
        if (x2 > 0 && y2 > 0)
        {
            ...
            else if (nImgId.find(_T("登")) != tstring::npos) //还在登录界面
            {
                ...
                if (!info.bLogin && m_dm.WaitStr(_T("密码"), _T("808080-3a3a3a"), 0.2) > PLUG_FAILED)
                {
                    LOG_INFO << "需要输入密码";
                    m_dm.SetWindowState(qqHwnd, 12);
                    Delays(200, 500);
...
void CQQLoginDlg::KPress(LPCTSTR key_str)
{
    generic_string str = key_str;
    unsigned long KeyDelay = CBaseScriptProperty::KeyDelay;
    double MoveDelayPer = CBaseScriptProperty::MoveDelayPer;
    ...
    for (auto ch : str)
    {
        ...
        TCHAR tempstr[3] = { 0 };
        tempstr[0] = ch;
        long key = strings::ChrToAsc(tempstr);
        if (key == 189)
        {
            key = vbKeySubtract;
        }
        ...
        {
            m_an.KeyPress(20);
            Delays((long)(KeyDelay * (1 - MoveDelayPer)), (long)(KeyDelay * (1 + MoveDelayPer)));
            m_an.KeyPress(key);
            Delays((long)(KeyDelay * (1 - MoveDelayPer)), (long)(KeyDelay * (1 + MoveDelayPer)));
            m_an.KeyPress(20);
        }
